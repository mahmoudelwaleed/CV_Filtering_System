...
"""
    ...
    # Configure RRF:
    # - formula_k=12: This is the 'k' from your original RRF function, used in the scoring.
    #                 A common value in literature is 60, you might want to experiment.
    # - top_n_limit=12: This will limit the number of documents returned by RRF to 12.
    #                   Change this value if you want a different number of final documents.
    ...
    def format_docs_with_metadata(docs_from_rrf): # Renamed from docs_with_scores for clarity
        if not docs_from_rrf:
            return "No relevant documents found after fusion."

        # The configured_rrf now returns a list of Document objects directly.
        ...
        for i, doc in enumerate(docs_to_format):
            ...
            if not hasattr(doc, 'metadata') or not hasattr(doc, 'page_content'):
                # Handle cases where an item might not be a full Document object
                # This might happen if retriever.map() or RRF somehow returns malformed items
                # Though unlikely with standard Langchain components
        ...
        return "\n".join(formatted_context) if formatted_context else "No relevant documents found after fusion and formatting."
...
No candidates match the criteria.
    ...
    rag_chain = (
        # The input to this chain is expected to be a dictionary: {"question": "user's question"}
